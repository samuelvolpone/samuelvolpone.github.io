...
# 
# 
# folds <- vfold_cv(df_analysis_unique, v = 10)
# 
# rf_wf <- 
#   workflow() %>%
#   add_model(rf_mod) %>%
...
#   fit_resamples(folds)
# 
# best_rf <- select_best(rf_fit_rs, metric = "roc_auc")
# 
...
# 
# results_rf <-
#   predict(rf_fit_rs, df_assessment_unique, type = "prob") %>%
#   pluck(2) %>%
#   bind_cols(df_assessment_unique, Predicted_Probability = .)
# 
# roc_auc(results_rf, truth = high_booking, Predicted_Probability, event_level = 'second')
# 
# roc_curve_rf <- roc(results_rf$high_booking, results_rf$Predicted_Probability)
# 
# # Plot ROC curve
...
```{r}
kaggle <- 
  predict(final_res, test_data, type = "prob") %>%
  pluck(2) %>%
  bind_cols(test_data, Predicted_Probability = .)

kaggle_submission <- kaggle %>% 
  select(unique_id, Predicted_Probability)

write.csv(kaggle_submission, "kaggle_submission14.csv", row.names = FALSE)
